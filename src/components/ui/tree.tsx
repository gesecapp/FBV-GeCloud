...
export const TreeExpander = ({ hasChildren = false, className, onClick, ...props }: TreeExpanderProps) => {
  ...
  return (
    <motion.div
      ...
      className={cn('mr-1 flex size-4 cursor-pointer items-center justify-center', className)}
      onClick={(e) => {
        e.stopPropagation();
        toggleExpanded(nodeId);
        onClick?.(e);
...
export const TreeIcon = ({ icon, hasChildren = false, className, ...props }: TreeIconProps) => {
  ...
  const getDefaultIcon = () => (hasChildren ? isExpanded ? <FolderOpen className="size-4" /> : <Folder className="size-4" /> : <File className="size-4" />);

  return (
    <motion.div
      className={cn('mr-2 flex size-4 items-center justify-center text-muted-foreground', className)}
